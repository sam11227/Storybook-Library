<script>
	...
	async function uploadFile(file) {
		...
		try {
			const response = await fetch('http://localhost:8080om/api/protected/uploadEventImage', {
				method: 'POST',
				headers: {
	...
	async function handleFiles(files) {
		...
		for (const file of Array.from(files)) {
			...
			error = '';
			dispatch('upload', { file, uploadedFileName });
		}
		...
		<div class="flex w-full gap-3.5">
			{#each images as image, index}
				<div class={`relative shadow ${shapeStyle(shape)}`}>
					...
					<img src={image} alt="uploaded image" class={`object-cover ${shapeStyle(shape)}`} />
					{#if !readonly}
						<button
			...
			{#each videos as video, index}
				<div class={`relative shadow ${shapeStyle(shape)}`}>
					...
					<video src={video} alt="uploaded video" class={`object-cover ${shapeStyle(shape)}`} />
					{#if !readonly}
						<button
							class={`${shapeClose(shape)} absolute bg-red-200 cursor-pointer`}
			...
			{#each pdfs as pdf, index}
				<div class={`relative shadow ${shapeStyle(shape)} flex justify-center items-center`}>
					<img
						src={pdfUpload}
						alt="PDF Icon"
						class={`"object-contain max-w-full max-h-full ${
							shape.includes('256x256-profile-photo') ? 'rounded-full' : 'p-2'
						}`}
					/>
					{#if !readonly}
						<button
			...
			{#if !readonly}
				<button
					class={`flex justify-center items-center border-dashed border-2 cursor-pointer border-brand-Primary border-opacity-50 bg-BG-Secondary ${shapeStyle(
						shape
					)}`}
