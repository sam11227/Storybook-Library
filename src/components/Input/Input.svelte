<script>
	...
	let inputValue = value; // Initialize inputValue with value
	$: if (inputValue !== value) inputValue = value; // Update inputValue when value changes
	...
	const handleInput = (event) => {
		...
		value = inputValue; // Propagate changes back to value
	};
...
<div class="flex flex-col gap-3 {className}">
	{#if label}
		<div class="flex justify-start items-center gap-1">
			<label for={id} class="text-Text-Primary text-sm not-italic leading-[14px] font-medium"
				>{label}</label
			>
			{#if optional}
			...
			<div class="flex items-center">
				...
				{#if controlled && (buttonIcon || buttonText)}
					...
					>
						...
						{/if}
						{#if buttonText}
							<span class="text-sm font-normal text-brand-Primary">{buttonText}</span>
						{/if}
					</button>
				{/if}
			</div>
