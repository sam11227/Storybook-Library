<script lang="ts">
	...
	import type { CheckboxItem, FormColorType } from '../types';
	import Label from './Label.svelte';
	import { inputClass, labelClass } from './Radio.svelte';
...
{#if choices.length > 0}
	{#each choices as { value, label }, i}
		<Label class={labelClass(inline, groupLabelClass)} show={$$slots.default} for={`checkbox-${i}`}
			>{label}
			<input
				id={`checkbox-${i}`}
				type="checkbox"
...
{:else}
	<Label class={labelClass(inline, $$props.class)} show={$$slots.default}>
		<input
			...
			{value}
			{...$$restProps}
			class={inputClass(custom, color, true, background, spacing, $$slots.default || $$props.class)}
		/>
		<slot />
	</Label>
{/if}
