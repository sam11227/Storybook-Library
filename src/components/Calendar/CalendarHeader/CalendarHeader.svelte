<script>
	...
	const handleNext = () => {
		...
		if (path.includes('/yearly')) {
			...
			year += 1;
			let date = $page.params.yearly.split('-')[1] + '-' + $page.params.yearly.split('-')[2];
	...
	$: {
		...
		if (path.includes('/yearly')) {
			nowDate = $page.params.yearly ? $page.params.yearly.split('-')[0] : currentYear.toString();
		} else if (path.includes('/monthly')) {
			const month = $page.params.month;
			if (month) {
				const [year, monthNumber] = month.split('-');
				const monthName = new Date(`${year}-${monthNumber}-01`).toLocaleString('default', {
					month: 'long'
				});
