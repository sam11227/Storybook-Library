	...
	const fetchEvents = async () => {
		...
		try {
			const response = await fetch(
				'http://localhost:8080/api/public/getEventsForVenue/' + currentVenueId
			);
			if (response.ok) {
...
>
	<header class="h-16 flex items-center w-full border-b border-stroke-Primary mx-auto">
		<nav class="w-full sm:max-w-6xl mx-auto flex items-center justify-between px-2">
			...
			<h2 class="text-[#0D1526] pb-5 pt-5 text-center text-xl leading-[20px] font-normal">
				{#if $currentView === 'Day'}
	...
	<main class="px-2 py-4 max-w-6xl mx-auto">
		{#if $currentView === 'Day'}
			...
			{:else}
				<DailyView selectedDate={$selectedDate} embedCalendar currentevents={events} />
			{/if}
		{:else if $currentView === 'Week'}
			{#if $loading}
				<SkeletonLoader type="weekly" />
			{:else}
				<WeeklyView selectedDate={$selectedDate} embedCalendar currentevents={events} />
			{/if}
		{:else if $currentView === 'Month'}
			{#if $loading}
		...
		{:else if $currentView === 'Quarter'}
			...
			{:else}
				<QuarterView year={$currentYear} quarter={$currentQuarter} currentevents={events} />
			{/if}
		{:else if $currentView === 'Year'}
			{#if $loading}
