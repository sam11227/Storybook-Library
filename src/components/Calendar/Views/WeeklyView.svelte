<script>
	import { addDays, format, getHours, startOfWeek } from 'date-fns';
...
<div class="overflow-hidden mx-auto w-full">
	...
	>
		...
		{#each weekDays as day}
			<div>
				<div class="day-header flex flex-col gap-2 border-l border-stroke-Primary uppercase">
	...
	<div class="grid grid-cols-8 gap-0 border-x border-stroke-Primary rounded-b-lg overflow-hidden">
		{#each Array(24)
			.fill()
			.map((_, i) => i) as hour}
			<div class="hour-cell flex justify-center items-center pt-3 pb-3 text-center p-2">
				{`${hour}:00`}
			</div>
			{#each weekDays as day}
						...
						<div class="event-badge text-xs mb-1">
							...
							{:else if embedCalendar}
								...
								<div
									...
									on:click={() => alert('You clicked on an event!')}
								>
									<img class="image" height="46px" src={event.image} alt={event.name} />
...
<style>
	...
	.image {
		...
		box-shadow: 0px 20px 6px 0px rgba(0, 0, 0, 0), 0px 13px 5px 0px rgba(0, 0, 0, 0.02),
			0px 7px 4px 0px rgba(0, 0, 0, 0.08), 0px 3px 3px 0px rgba(0, 0, 0, 0.14),
	...
	.embedCalendar {
		box-shadow: 0px 56px 16px 0px rgba(43, 43, 43, 0), 0px 36px 14px 0px rgba(43, 43, 43, 0.01),
			0px 20px 12px 0px rgba(43, 43, 43, 0.05), 0px 9px 9px 0px rgba(43, 43, 43, 0.09),
