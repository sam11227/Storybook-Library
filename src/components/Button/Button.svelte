<script lang="ts">
  ...
  $: sizeClass = (() => {
    switch (size) {
      ...
      case 'sm': return 'px-3 py-2 leading-4 rounded-md';
...
<button
  class={classNames(
    'cursor-pointer px-3 py-3 inline-flex items-center rounded-lg shadow-sm focus:outline-none no-focus-ring flex justify-center',
    sizeClass,
    alert ? (strokebtn ? 'btn-alert-stroke' : 'btn-alert') : (strokebtn ? 'btn-default-stroke' : 'btn-default'),
    deemphasized ? (strokebtn ? 'btn-deEmphasized-outline' : 'btn-deEmphasized') : '',
    lightbtn ? (strokebtn ? 'btn-light-outline' : 'btn-light') : '',
    danger ? (strokebtn ? 'btn-danger-stroke' : 'btn-danger') : '',
    strokebtn ? 'btn-text-style-outline' : ScheduledBtn ? 'scheduledBtn-text-style' : 'btn-text-style',
    textBtn ? 'btn-text' : '',
...
>
  {#if beforeIcon}<img src={beforeIcon} alt="Before Icon" class="mr-2" />{/if}
  {#if label}<span>{label}</span>{:else}<slot />{/if}
  {#if afterIcon}<img src={afterIcon} alt="After Icon" class="ml-2" />{/if}
</button>
