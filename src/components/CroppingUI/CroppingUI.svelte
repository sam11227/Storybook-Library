<script>
	...
	async function getRotatedImage(imageSrc, rotation) {
		...
		canvas.width = Math.abs(Math.cos(angle) * width) + Math.abs(Math.sin(angle) * height);
		canvas.height = Math.abs(Math.sin(angle) * width) + Math.abs(Math.cos(angle) * height);
		...
		>
			...
			<div class="w-full flex justify-end mt-6">
				<div class="flex items-center gap-1.5 w-52">
					<Button strokebtn type="button" size="full" on:click={handleCloseEditModal}>
						Cancel
					</Button>
